...
//        sc = new Scanner(System.in);
    ...
    private static void selectMenu() {
        ...
        switch (menuNum) {
            ...
            case "5":
                // 사용자에게 검색어를 입력받는다.
                String author_name = input("# 검색어: ");

                // 저장소에게 해당 검색어를 주면서 검색에
                // 걸린 책 정보를 내노라고 해야함
                String[] bookInfoList_author = repository.searchAuthorList(author_name);
                if (bookInfoList_author.length > 0) {
                    System.out.printf("\n======== [%s] 검색 결과 =========\n", author_name);
                    for (String info : bookInfoList_author) {
                        System.out.println(info);
                    }
                } else {
                    System.out.println("\n# 검색 결과가 없습니다.");
